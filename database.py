...
async def get_leaderboard() -> str:
    ...
    for i in range(0, len(top_users), 10):
        ...
        lines = [
            f"> • **{str(i + j + 1).zfill(2)}. {user['username']} - `{user['points']}` Shards**"
...
async def add_shards(userid: int, shards: int) -> dict:
    ...
    if updated.matched_count == 0:
        return {"success": False, "message": "User not found!", "color": RED}
    return {"success": True, "message": f"{shards} shards added to <@{userid}>!", "color": GREEN}
...
async def remove_shards(userid: int, shards: int) -> dict:
    ...
    if updated.matched_count == 0:
        return {"success": False, "message": "User not found!", "color": RED}
    return {"success": True, "message": f"{shards} shards removed from <@{userid}>!.", "color": GREEN}

## User info
async def get_user(userid: int) -> dict:
    user_details = await userdata.find_one({'userid': userid})
    if user_details:
        return {"success": True, "message": user_details, "color": GREEN}
    else:
        return {"success": False, "message": "User not Found.", "color": RED}
...
async def daily_checkin(userid: int, username: str) -> dict:
    ...
    if user:
        ...
        if last_checkin == today_str:
            return {"success": False, "message": f"Hey <@{userid}> you've already checked in today.", "color": RED}
    ...
    return {"success": True, "message": f"Congratulations <@{userid}> check-in successfull! You received {earned} shards.", "color": GREEN}
...
async def get_daily_quiz(user_id: int) -> dict:
    ...
    if unseen_quiz:
        quiz = unseen_quiz[0]
        return {"success": True, "message": quiz, "color": GREEN}
    else:
        return {"success": False, "message": f"Hey <@{user_id}> seems like you've finished all the available quizzes.", "color": RED}
...
async def submit_answer(user_id: int, quiz_id: str, selected_answer: str) -> dict:
    ...
    if correct:
        ...
        return {"success": True, "message": f"Congratulations you won {points_awarded} Shards!", "color": GREEN}
    else:
        update = {
            "$inc": {
                "quiz_data.total_quizzes": 1,
        ...
        await userdata.update_one({"userid": user_id}, update)
        return {"success": False, "message": f"Oops <@{user_id}>! Your answer was incorrect. Better luck next time!", "color": RED}
...
async def add_remove_role(user_id: int, role_id: str, reward: int, remove: bool) -> None:
    if not remove:
        ...
        return {"success": True, "message": f"Added role: {role_id} to User: {user_id}", "color": GREEN}
    else:
        user = await userdata.find_one({"userid": user_id})
        roles = user.get("roles", {})
        if role_id in roles:
...
async def claim_role_reward(user_id: int, role_id: str) -> dict:
    ...
    return {"success": True, "message": f"Congratulations <@{user_id}> you've successfully claimed {reward_amount} Shards for <@&{role_id}> role.", "color": GREEN}
